...
export function InvoiceDocument({ invoice, editable = false, onItemsChange }: InvoiceDocumentProps) {
  ...
  function addItem() {
    onItemsChange?.([
      ...invoice.line_items,
      { id: crypto.randomUUID(), description: "Yeni hizmet", quantity: 1, unit_price: 0 }
    ]);
  }
    ...
    <div className="rounded-xl border bg-white p-6">
      <div className="mb-6 flex items-start justify-between">
        <div>
          ...
          <p className="mt-[2px] text-[12px] leading-[1.6] text-text-secondary">
        ...
        <div className="text-right">
          ...
          <span className={cn("mt-2 inline-flex rounded-full px-3 py-1 text-[11px] font-medium leading-[1.6]", status.className)}>
            {status.label}
          </span>
        </div>
      </div>
      ...
      <div className="mb-5 grid grid-cols-2 gap-4">
        <div>
          ...
          <p className="text-[12px] leading-[1.6] text-text-secondary">{invoice.freelancer_email}<br />{invoice.freelancer_city}</p>
        </div>
        <div>
          ...
          <p className="text-[12px] leading-[1.6] text-text-secondary">{invoice.client_email}<br />{invoice.client_company}</p>
        </div>
      </div>
          ...
          <tr>
            {["Hizmet", "Adet", "Birim fiyat", "Toplam"].map((heading, index) => (
              <th
                key={heading}
                className={cn(
                  "border-b px-2 py-[6px] text-[11px] font-medium uppercase leading-[1.6] tracking-wide text-text-secondary",
                  index > 0 && "text-right"
                )}
              >
                {heading}
          ...
          {invoice.line_items.map((item) => (
            <tr key={item.id} className="group">
              <td className="border-b px-2 py-[9px] text-[12px] text-text-primary">
                {editable ? (
                  <input className="w-full bg-transparent outline-none" value={item.description} onChange={(event) => updateItem(item.id, { description: event.target.value })} />
                ) : (
                  item.description
                )}
              </td>
              <td className="border-b px-2 py-[9px] text-right text-[12px]">
                {editable ? (
                  <input className="w-12 bg-transparent text-right outline-none" type="number" min="1" value={item.quantity} onChange={(event) => updateItem(item.id, { quantity: Number(event.target.value) })} />
                ) : (
                  item.quantity
                )}
              </td>
              <td className="border-b px-2 py-[9px] text-right text-[12px]">
                {editable ? (
                  <input className="w-24 bg-transparent text-right outline-none" type="number" min="0" value={item.unit_price} onChange={(event) => updateItem(item.id, { unit_price: Number(event.target.value) })} />
                ) : (
      ...
      <div className="flex items-center justify-between rounded-xl bg-amber-50 px-[14px] py-[10px]">
